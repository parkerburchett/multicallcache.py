...
def main():
    ...
    calls = [Call(weth, "balanceOf(address)(uint256)", (a), f"{a}_weth_bal", toETH, w3) for a in to_addresses]
    before_building_multicall = datetime.now()
    multi = Multicall(calls, w3)
    print(
        datetime.now() - before_building_multicall,
        "time taken to construct multicall object (calldata encoding dominating cost)",
    )
